...
const createUser = async (userBody) => {
  try {
    ...
    console.log('createUser error occured', error);
...
const updateUserById = async ({ value }) => {
  const user = await getUserById(value.id);
  if (!user) {
    throw new ApiError(httpStatus.NOT_FOUND, 'User not found');
  }
  if (value.email && (await User.isEmailTaken(value.email, value.id))) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Email already taken');
  }
...
const setPrompt = async (userId, { prompt }) => {
  const prompts = await Prompt.findOne({ userId });
  const prompt_data = {
    prompt,
    userId,
  };
  Object.assign(prompts, prompt_data);
  await prompts.save();
  return prompts;
};

const getPrompt = async (userId) => {
  return await Prompt.findOne({ userId });
};
