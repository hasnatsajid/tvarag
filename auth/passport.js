...
passport.use(
  ...
  new LocalStrategy(
    ...
    async (req, emailInput, passwordInput, done) => {
      const { full_name, user_name, role, phone_number } = req.body;
      const payload = {
        full_name,
        user_name,
        email: emailInput,
...
passport.use(
  ...
  new LocalStrategy({ usernameField: 'email', passwordField: 'password' }, async (email, password, done) => {
    ...
    console.log(user);
    const { _id: id, user_name, role, full_name } = user;

    const options = {
      expiresIn: 3600,
    };

    const token = generateJWTToken({ id, user_name, role, full_name }, options);
