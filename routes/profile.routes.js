...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    res.status(200).json(await updateUserById(req.body));
  })
);
...
router.get(
  ...
  passport.authenticate('jwt', { session: false }),
  catchAsync(async (req, res) => {
    console.log('/api/profile/getPrompt called -------');
    res.status(200).json(await getPrompt(req.user._id));
  })
);
...
router.post(
  ...
  passport.authenticate('jwt', { session: false }),
  catchAsync(async (req, res) => {
    console.log('/api/profile/setPrompt called -------');
    res.status(200).json(await setPrompt(req.user._id, req.body));
  })
);
